...
public class JsonCommentDtoTests {
    ...
    void testComment() throws Exception {
        ...
        Comment comment = new Comment(1, "Комментарий к букингу", new Item(), "Вася", created);
        CommentDto commentDto = CommentMapper.toCommentDto(comment, new User());

        CommentDto commentDtoNew = new CommentDto(2, "Комментарий к букингу из ДТО", new Item(), "Petr", created);
    ...
    void testCommentResponseDto() throws Exception {
        LocalDateTime created = LocalDateTime.now().plusDays(1);
        Comment comment = new Comment(1, "Комментарий к букингу",
                new Item(2, "Веник", "Домашний",
                        new User(), true, 3), "Вася", created);
